...
class TranslationDataset(Dataset):
    def __init__(self, split: str, max_len: int, max_vocab: int = 20000, cache_vocabs: Dict = None, seed: int = 42):
        """
        split: 'train', 'validation', or 'test'
        """
        self.ds = load_dataset("opus_books", "en-fr")
        data = list(self.ds["train"])
        ...
        if split == "train":
            self.samples = data[:train_size]
        elif split == "validation":
            self.samples = data[train_size:train_size + val_size]
        else:  # test
            self.samples = data[train_size + val_size:]
